...
void Store::set(const std::string& key, const std::string& value,
  ...
  if (px_ms > 0) {
    ...
    entry.expiry =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(px_ms);
  }
...
int Store::rpush(const std::string& key,
...
bool Store::lrange(const std::string& key, int start, int end,
...
int Store::lpush(const std::string& key,
                 const std::vector<std::string>& values) {
  auto it = db.find(key);
